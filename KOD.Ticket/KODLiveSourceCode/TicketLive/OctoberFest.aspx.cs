...
public partial class OctoberFest : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            if (listPromo[i].PromotionCode.ToString().ToUpper() == "OCTOBERFEST")
            {
    ...
    protected void ddl_ShowTimes_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        KoDTicketingLibrary.DTO.Promotion PromoSession = (KoDTicketingLibrary.DTO.Promotion)Session["a" + "OCTOBERFEST"];
    ...
    {
        const ushort MAX_SEATS_PER_TRANSACTION = 2; //needs to be moved to config;
        drp_TotalSeats.Items.Clear();
        drp_TotalSeats.Items.Add(new ListItem("Select", "0"));
        ...
        int availableSeats = VistaBOL.Select_Available_Seats(Category, PlayTime);
            if (availableSeats > 0) //if there are seats available...
            {
                ...
                availableSeats = (availableSeats > MAX_SEATS_PER_TRANSACTION) ? 2 : availableSeats;
                for (int i = 2; i <= availableSeats; i++)
