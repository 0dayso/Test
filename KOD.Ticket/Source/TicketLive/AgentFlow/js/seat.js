    ...
    for (i = 0; i < chkseatbook.length; i++) {
        if (!chkseatbook[i])
            ...
            if (chekedarr.length > 0) {
                for (var j = 0; j < chekedarr.length; j++) {
                    if (chekedarr[j] == chkseatbook[0]) {
                        pos = j;
                        chkID = 1;
                        break;
                    }
            ...
            if (chkID == 1) {
                getDoc(chekedarr[pos]).firstChild.src = "../Images/W_chair.gif";
                for (var i = pos; i < chekedarr.length - 1; i++) {
                    chekedarr[i] = chekedarr[i + 1];
                }
                chekedarr.pop();
                _seatCoun--;
                break;
                ...
                getDoc(chkseatbook[0]).firstChild.src = "../Images/G_chair.gif";
                chekedarr.push(chkseatbook[0]);
                _seatCoun++;
                break;
            }
        }
