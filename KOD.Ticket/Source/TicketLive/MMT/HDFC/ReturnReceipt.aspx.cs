...
public partial class MMT_HDFC_ReturnReceipt : System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            if (ErrorNo == String.Empty)
            {
                ...
                if (trackid != String.Empty)
                {
                    ...
                    {
                        string redirectURL = "REDIRECT=" + KoDTicketingIPAddress + "MMT/Print-Receipt.aspx" + qstring;
                ...
                Response.Write("REDIRECT=" + KoDTicketingIPAddress + "MMT/Print-Receipt.aspx?err=pay");
            }
            else
            {
    ...
    protected String UpdateResponse(String trackid, String reference, String result, String postdate, String auth)
    {
        ...
        try
        {
            ...
            Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write(string.Format("HDFC Payment Response: Reference[{0}], Booking ID [{1}], Agent Code [{2}] ", tr.ReferenceNo.ToString(), tr.MMTBookingID.ToString(), tr.AgentCode));
            ...
            if (Request["paymentid"] != null)
            {
                try
                {
                    ...
                    tr.MMTReceiptNo = Request["paymentid"].ToString();
                }
                catch (Exception ex)
                {
            ...
            if (result == "CAPTURED")
            {
                ...
                try
                {
                    string MMTBookingID = tr.MMTBookingID.ToString();
                    TransactionBOL.Get_MMT_Detail(tr.MMTBookingID, tr.MMTReceiptNo);
                    Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write("Going to Fetch Details of Booking Id from DB.Booking ID where" + MMTBookingID);
                    DataTable dt = TransactionBOL.Select_MMTTransaction_REFIDWISE(tr.MMTBookingID.ToString());
                    Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write("Fetched Details of Booking Id from DB.Booking ID where" + MMTBookingID);
                    if (dt != null && dt.Rows.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            qstring.Append("?b=");
                            qstring.Append(tr.MMTBookingID.ToString());
                            ...
                            Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write("HDFC Payment successful..." + tr.MMTBookingID.ToString() + tr.MMTReceiptNo.ToString());
                    ...
                    else
                    {
                        ...
                        //ReceiptUtils.SuccessPaymentResponse(tr.MMTBookingID.ToString(), tr.MMTReceiptNo);
                ...
                KoDTicketing.GTICKV.LogEntry(_refNo, "HDFC Payment Not Successful", "17", tr.MMTBookingID.ToString());
                //GTICKBOL.ON_Session_out(_refNo);
                KoDTicketing.GTICKV.LogEntry(_refNo, "Seats Unlocked", "18", tr.MMTBookingID.ToString());
                qstring.Append("err=pay");

                KoDTicketing.GTICKV.LogEntry(tr.MMTReceiptNo.ToString(), "HDFC Error Occurred -- Payment Not Successful", "19", tr.MMTBookingID.ToString());
                ...
                Response.Redirect(IpAddress + "MMT/HDFC/FinalCall.aspx" + qstring.ToString(), false);
                #endregion
            }
            else
            {
                GTICKV.LogEntry(tr.MMTReceiptNo.ToString(), "HDFC Payment Not Successful", "17", tr.MMTBookingID.ToString());
                //GTICKBOL.ON_Session_out(tr.ReferenceNo.ToString());
                GTICKV.LogEntry(tr.MMTReceiptNo.ToString(), "Seats Unlocked", "18", tr.MMTBookingID.ToString());
