...
namespace KoDTicketing.DataAccessLayer
{
    public class TransactionDAL : DBAccess
    {
        ...
        public static int _Transaction_Temp_Insert(TransactionRecord tr)
        {
            ...
            if (tr.AgentCode.ToString() == "WEB")
                command.Parameters.AddWithValue("@IsProcessed", tr.IsProcessed);
            else
                command.Parameters.AddWithValue("@IsProcessed", DBNull.Value);
            if (tr.AgentCode.ToString() == "WEB")
        ...
        public static DataTable _Get_Transaction_Detail(TransactionRecord tr)
        {
            ...
            try
            {
                ...
                do
                {
                    ...
                    if ((AgentCodeTokens.Length == 1 && AgentCodeTokens[0] == "WEB") || AgentCodeTokens[0] == "Agent")
                    {
                    ...
                    if (chkdata == 0)
                    {
                        ...
                        if (dt != null && dt.Rows.Count > 0 &&(Convert.ToDateTime(dt.Rows[0]["DateOfBooking"].ToString()) ==Convert.ToDateTime(DateTime.Now.Date.ToShortDateString())||Convert.ToDateTime(dt.Rows[0]["DateOfBooking"].ToString()) ==Convert.ToDateTime(DateTime.Now.Date.AddDays(-1).ToShortDateString())))
                        {
                            try
                            {
                                ...
                                {
                                    if (AgentCodeTokens[0] == "Agent")
                                    {
                                        AgentCode = AgentCodeTokens[1];
                                        AgentCodeSubcode = AgentCodeTokens[2] != null ? AgentCodeTokens[2].ToString() : "";
                                    }
                                    else
                                    {
            ...
            try
            {
                ...
                if (exisitng_ticket_booked_count == 0 && tr.PromotionCode == "FAMILYOFFER")
                {
                    SqlCommand comand = new SqlCommand();
                    comand.CommandText = "Update [dbo].[tbl_familyoffer] set IsPaymentSuccess=1 ,[ReceiptId]='" + tr.ReceiptNo.ToString() + "' where [BookingId] = '" + tr.ReferenceNo.ToString() + "'";
                    Connection.EXECommand(comand, connMSTicket);
                }
                ...
                if (exisitng_ticket_booked_count == 0&&tr.AgentCode.Split('-')[0]!="Agent")
                {
        ...
        public static DataSet _Select_Report_FromTransactionTable(TransactionRecord tr)
        {
            ...
            SqlCommand command = new SqlCommand();
            string sqlQuery = "Select [Category],SUM([TotalSeats]) as [No Of Seats],SUM([TotalAmount]) as 'Total Amount',SUM(CAST(Round(TotalAmount*((100-DiscountPercentage)/100),0) as decimal(18,2))) as [Discounted Amount] from [GINC$BookingTransaction] " +
                " where  id > 0 ";
            if (tr.AgentCode == "1")
            ...
            sqlQuery += " GROUP BY [Category]  order by [Category] Select SUM([TotalSeats]) as [TotalSeats], SUM([TotalAmount])as TotalAmount" +
                ",SUM(CAST(Round(TotalAmount*((100-DiscountPercentage)/100),0) as decimal(18,2))) as [Discounted Amount],PaymentGateway  from [GINC$BookingTransaction] where id > 0 ";
            if (tr.AgentCode == "1")
                sqlQuery += " and AgentCode = 'WEB' ";
            if (tr.AgentCode == "2")
                sqlQuery += " and AgentCode != 'WEB' ";
 ...
 "CAST(a.[DiscountPercentage] as decimal(18,2)) as 'Discounted %age',CAST(Round(a.TotalAmount*((100-a.DiscountPercentage)/100),0) as decimal(18,2)) as [Discounted Amount],a.PaymentGateway as [Card Type],a.[Name] as [Customer Name],a.[MobileNo],a.[EmailID],Replace(a.[SeatInfo],',',', ') as [SeatInfo], a.[AgentCode],b.ErrorTxt,f.[Error_Detail] as [Utility Result] ";
        ...
        public static DataSet Detailed_Report(DateTime startDateOfBooking, DateTime EndDateOfBooking, string AgentCode, string Play)
        {
            ...
            {
                if (connMSTicket.State != ConnectionState.Closed)
                {
                    connMSTicket.Close();
                }
            }
        ...
        public static DataSet Select_Report_tbl_NewYearPackages(string BookingID, string BookingDateFrom, string BookingDateTo, string Name, string pgReceipt, string Package, int paymentStatus)
        {
            string sqlQuery = "SELECT row_number() over (order by ID desc) as [S.N.],[BookingId] as [Booking ID],[Qty_PackageTypeCouple] as Couple,[Qty_PackageTypeSingle] as Single,[Qty_PackageTypeTeen] as Teens,[Qty_PackageTypeKid] as Kids, " +
                    ...
                    "from dbo.tbl_NewYearPackages where ID > 0 and YEAR([DateOfBooking])=2013";
        ...
        public static DataTable _Select_MMTTransaction_REFIDWISE(string MMTBookingID)
        {
            ...
            Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write("Fetch Details of Booking Id from DB.Booking ID where" + MMTBookingID + command.CommandText);
            return Connection.readTab(command, connMSTicket);
        }
        ...
        public static DataTable _Select_FAMILYOFFERTransaction_REFIDWISE(string FAMILYOFFERBookingID)
        {
            SqlCommand command = new SqlCommand();
            command.CommandText = "Select * from [dbo].[tbl_familyoffer] where [FAMILYOFFERBookingId] = '" + FAMILYOFFERBookingID.ToString() + "'";
            Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write("Fetch Details of Booking Id from DB.Booking ID where" + FAMILYOFFERBookingID + command.CommandText);
            return Connection.readTab(command, connMSTicket);
        }
        ...
        internal static void Get_Event_Details(string BookingId, string ReceiptNo)
        {
            SqlCommand command = new SqlCommand();
            command.CommandText = "Update [dbo].[Event_Transaction] set PGIsPaymentSuccess=1 ,[PGReceiptId]='" + ReceiptNo.ToString() + "' where [BookingID] = '" + BookingId.ToString() + "'";
            Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write("Update Details of PGIsPaymentSuccess and PGReceiptId from DB where Booking ID:" + BookingId + "Receipt No." + ReceiptNo);
            Connection.EXECommand(command, connMSTicket);
        }
        ...
        internal static DataTable Select_EventTransaction(string EvtBookingID)
        {
            SqlCommand command = new SqlCommand();
            command.CommandText = "Select * from [dbo].[Event_Transaction] where [BookingID] = '" + EvtBookingID.ToString() + "'";
            Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write("Fetch Details of Booking Id from DB.Booking ID where" + EvtBookingID + command.CommandText);
            return Connection.readTab(command, connMSTicket);
        }
        internal static DataTable Check_BotyTransaction(string entryid)
        {
            SqlCommand command = new SqlCommand();
            command.CommandText = "Select * from [dbo].[tbl_Boty] where [BookingID] = '" + entryid.ToString() + "'";
        ...
        internal static DataTable Check_EventTransaction(string BookingID)
        {
            SqlCommand command = new SqlCommand();
            command.CommandText = "Select * from [dbo].[Event_Transaction] where [BookingID] = '" + BookingID.ToString() + "'";
            Microsoft.Practices.EnterpriseLibrary.Logging.Logger.Write("Fetch Details of Booking Id from DB.Booking ID where" + BookingID + command.CommandText);
            return Connection.readTab(command, connMSTicket);
        }
...
"sum(case when [ShowDate]='"+ShowDate1+"' and [AuditNo]=2 and Iscompleted="+Iscompleted+" and [Status]='" + status + "'and [ShowLocation]='"+ShowLocation+"' then 1 else 0 end) as AuditNo2,[Category] FROM [dbo].[ShowAudit_Table] where [Category] is not null group by Category";
        ...
        public static string Successful_BookingByMsTicket(decimal Amount, string Play, string CustomerNo, string referenceID, int TotalSeats, string Gateway, string AgentCode, decimal discountPercent, string ReceiptNo, decimal RoyalAmount, decimal RoyalPoints, string BookingID, string AgentCodeSubcode, string WebPromotionID)
        {
            try
            {
                ...
                //,string Gateway,string AgentCode,decimal discountAmt,decimal discountPercent
